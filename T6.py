...
def fsolve2(func, x0, args, tol=1e-4, max_iter=10):
    ...
    for _ in range(max_iter):
        ...
        delta_x = np.linalg.solve(jacobian, -f_val)
        x += delta_x
...
class State:
    ...
    def plot(x="t2", y=("x", "v", "a"), **kw):
        x_vals = eval(f"s.{x}_vals")
        y_vals = (eval(f"s.{y_val}_vals") for y_val in np.atleast_1d(y))
        savefig = kw.pop("savefig", False)

        fig, ax = plt.subplots()
        for t_val, y_val in zip(y_vals, np.atleast_1d(y)):
            ax.plot(x_vals, t_val, label=y_val, **kw)
        ax.set_xlabel(x)
        ax.set_ylabel(str(y).replace("'", "").replace("[", "").replace("]", ""))
