...
def loop1_displacement(vars, theta2):
    theta3, theta41 = vars
    return (1 * cos(theta2) + 4 * cos(theta3) - 3 * cos(theta41) - O4x,
            1 * sin(theta2) + 4 * sin(theta3) - 3 * sin(theta41) - O4y)
def loop1_velocity(vars, omega2, theta2, theta3, theta41):
    omega3, omega41 = vars
    return (omega2 * -sin(theta2) + 4 * omega3 * -sin(theta3) - 3 * omega41 * -sin(theta41),
            omega2 * cos(theta2) + 4 * omega3 * cos(theta3) - 3 * omega41 * cos(theta41))
def loop1_acceleration(vars, alpha2, omega2, theta2, omega3, theta3, omega41, theta41):
    alpha3, alpha41 = vars
    return (omega2**2 * -cos(theta2) + alpha2 * -sin(theta2) + 4 * (omega3**2 * -cos(theta3) + alpha3 * -sin(theta3)) - 3 * (omega41**2 * -cos(theta41) + alpha41 * -sin(theta41)),
            omega2**2 * -sin(theta2) + alpha2 * cos(theta2) + 4 * (omega3**2 * -sin(theta3) + alpha3 * cos(theta3)) - 3 * (omega41**2 * -sin(theta41) + alpha41 * cos(theta41)))
def loop2_displacement(vars, theta4):
    theta5, distx = vars
    return (3 * cos(theta4) + 3.2 * cos(theta4 + anggggle) - 6.5 * cos(theta5) - (distx - O4x),
            3 * sin(theta4) + 3.2 * sin(theta4 + anggggle) - 6.5 * sin(theta5) - (3 - O4y))
def loop2_velocity(vars, omega4, theta4, theta5):
    omega5, velx = vars
    return (3 * omega4 * -sin(theta4) + 3.2 * omega4 * -sin(theta4 + anggggle) - 6.5 * omega5 * -sin(theta5) - velx,
            3 * omega4 * cos(theta4) + 3.2 * omega4 * cos(theta4 + anggggle) - 6.5 * omega5 * cos(theta5))
def loop2_acceleration(vars, alpha4, omega4, theta4, omega5, theta5):
    alpha5, accx = vars
    return (3 * (omega4**2 * -cos(theta4) + alpha4 * -sin(theta4)) + 3.2 * (omega4**2 * -cos(theta4 + anggggle) + alpha4 * -sin(theta4 + anggggle)) - 6.5 * (omega5**2 * -cos(theta5) + alpha5 * -sin(theta5)) - accx,
            3 * (omega4**2 * -sin(theta4) + alpha4 * cos(theta4)) + 3.2 * (omega4**2 * -sin(theta4 + anggggle) + alpha4 * cos(theta4 + anggggle)) - 6.5 * (omega5**2 * -sin(theta5) + alpha5 * cos(theta5)))
def theta3_4():
